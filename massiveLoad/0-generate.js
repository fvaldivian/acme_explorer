...
for (let i = 0; i < explorersIds.length; i++) {
  ...
  const low_price = Number(
    faker.commerce.price((/* min */ minimunFinderPriceLowerBound)),
    (/* max */ maximunFinderPriceLowerBound)
  );
  const high_price = Number(
    faker.commerce.price((/* min */ low_price + 1)),
    (/* max */ maximunFinderPriceUpperBound)
  );
  const from_date = faker.date.future(
    (/* years */ 3),
    (/* refDate */ threeYearsAgo)
  );
  const to_date = faker.date.future(
    (/* years */ 3),
    (/* refDate */ from_date)
  );
  ...
  const results = allTrips.filter(function (trip) {
    const priceMatchs =
      low_price <= trip.price && trip.price <= high_price;
    const dateMatches =
      new Date(from_date) <= new Date(trip.startDate) &&
      new Date(trip.endDate) <= new Date(to_date);
